...
var HTTPServer = exports.HTTPServer = function (options) {
  ...
  function addReloadScript(src, contentType)
  {
    var index = src.indexOf("</body");
    if (src.trim().length === 0 && contentType.indexOf("text/html") != -1) {
      // add reload script to blank HTML files aggresively, in case the file changes after a while.
      return '<html><head></head><body><script type="text/javascript" src="http-hot-reload.js"></script></body></html>';
    }
    else if (index == -1) {
      return src;
    }
